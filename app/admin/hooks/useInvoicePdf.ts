...
export default function useInvoicePdf({
  ...
  const generatePdf = useCallback(() => {
    ...
    if (items.length === 0) {
      ...
      for (const item of items) {
        ...
        doc.text(Number(item.qty ?? 0).toFixed(2), 120, y, { align: 'right' })
  ...
  }, [invoice, items, businessDetails, paymentDetails, formatCurrency, formatDateTime])
