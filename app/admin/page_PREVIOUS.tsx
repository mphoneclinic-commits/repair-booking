...
repair_performed: updatedJob.repair_performed ?? '',
...
async function updateRepairPerformed(id: string, value: string) {
  setFieldState(id, 'repair_performed', 'saving')

  const { data, error } = await supabase
    .from('repair_requests')
    .update({ repair_performed: value || null })
    .eq('id', id)
  ...
  async function createInvoiceForJob(job: RepairRequest) {
    ...
    const defaultDescription = job.repair_performed?.trim() || `Repair service for ${job.brand} ${job.model}`.trim()
